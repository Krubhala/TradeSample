...
public class TradeExample {
    ...
    public static void main(String[] args) {
		...
		double exchange = 0;
		int units = 0;
		File file=new File("C:\\Project\\trade.txt");
	  ...
	  private static HashMap<Date, Double> getTotalUSD(ArrayList<Trade> tradeList){
		  ...
		  for(Trade tr : tradeList) {
	          ...
	          if ("AED".equals(tr.getCurrency().toUpperCase()) || "SAR".equals(tr.getCurrency().toUpperCase())) {
	        	  d1=6;// For AED/SAR Friday weekend 
	        	  d2=7;// For AED/SAR Saturday weekend
			  }	
	          
	          while(!isBankHoliday(c.getTime(), c, d1, d2)) {
	  ...
	  public static boolean isBankHoliday(Date date, Calendar calendar, int d1, int d2) {
		  calendar.setTime(date);
          int dayOfWeek = calendar.get(Calendar.DAY_OF_WEEK);
          // check if it is Saturday(day=7) or Sunday(day=1) - For currencies other than AED and SAR
          // check if it is Friday(day=6) or Saturday(day=7) - For currencies AED and SAR
